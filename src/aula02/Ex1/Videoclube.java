...
public class Videoclube {
	...
	public static void addPessoa(socio[] socio) {
		...
		do {
			...
			if (tipo.equals("e") || tipo.equals("E")) {
				...
				p1 = new Pessoa(nome, cc, nasc, nMec, curso);
				...
				p1 = new Pessoa(nome, cc, nasc, nFunc, nFisc);
			}
		} while (check == false);

		socio[I++] = new socio(p1);
	}

	public static void removePessoa(socio[] socio) {

		System.out.println("Número de sócio:");
		int numSocio = sc.nextInt();

		for (int i = 0; i < I; i++) {
			if (numSocio == socio[i].getNumSocio()) {

				System.out.printf("Utilizador %s eliminado!\n", socio[i].getPessoa().getNome());
	...
	public static void checkOut(socio[] socio, video[] filmes) {
		...
		for (int k = 0; k < i; k++) {
			if (id == filmes[k].getID()) {
				for (int j = 0; j < I; j++) {
					if (numSocio == socio[j].getNumSocio()) {
						boolean disponivel = false;

						filmes[k].setDisponivel(disponivel);
						socio[j].setVideos(filmes[j]);
	...
	public static void checkIn(socio[] socio, video[] filmes) {
	...
	public static void identifyUser(video[] filmes) {
		...
		for (int k = 0; k < i; k++) {
			if (id == filmes[k].getID()) {
				...
				socio[] socio = filmes[k].getSocio();

				for (int j = 0; j < filmes[k].getZ(); j++) {
					System.out.print("Cliente: \n");
					System.out.println(socio[j].getPessoa().toString());
				}
			}
		}
	}
	...
	public static void historicoVideosEmprestados(socio[] socio, video[] filmes) {
		...
		for (int k = 0; k < I; k++) {
			if (numSocio == socio[k].getNumSocio()) {
				v = socio[k].getVideos();
